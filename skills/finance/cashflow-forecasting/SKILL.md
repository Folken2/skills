...
1. **Establish the opening cash position.** Start from actual reconciled bank balances across all accounts. Adjust for known-but-uncleared items (in-flight transfers, uncashed payments) to get true **available** cash, and separate any restricted or committed funds. A forecast built on an unreconciled opening balance is wrong from week one.
...
4. **Build the 13-week rolling grid.** Lay out 13 weekly columns: opening cash + inflows − outflows = closing cash, carried as next week's opening. Weekly granularity (not monthly) is what exposes an *intra-month* trough that a monthly view hides. Document every assumption (collection lag %, payment timing) alongside the numbers so they can be challenged and corrected.
...
7. **Run variance analysis and reforecast.** Each week, compare last week's forecast to actuals line by line. Classify each variance as **timing** (cash moved weeks, total unchanged) vs. **permanent** (the amount was wrong), because they demand different fixes. Correct the assumptions, then roll the window forward one week so it always shows the next 13.
