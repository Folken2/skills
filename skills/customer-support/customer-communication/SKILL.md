...
description: "Use when writing or improving any customer-facing support message — setting tone, structuring a reply, adapting to channel, or handling difficult conversations like complaints, delays, denials, and escalations. Triggers on 'how do I word this', 'help me reply', 'the customer is angry', 'how do I say no', 'de-escalate', 'break bad news', 'reply to this review'."
version: 1.0.0
author: Nuvel Skills
---
...
The same resolution can leave a customer loyal or furious depending entirely on how it was communicated. The core principle: **lead with empathy and clarity, own the situation without over-promising, and always leave the customer knowing what happens next.** Good support writing is not scripted politeness — it is understanding shown, action stated, and expectations set honestly.
...
- The conversation is difficult: a complaint, a delay, a denied request, or a pricing objection.
- You need to say "no" or deliver bad news without losing the relationship.
- You are escalating and need language that is firm but respectful.

## When NOT to use

- You have not yet worked out *what* to tell the customer (the diagnosis or decision) → resolve that with [[technical-troubleshooting]] or [[order-management]] first, then return here for the wording.
- The ticket needs classifying, prioritizing, or routing → use [[ticket-triage]].
...
- **Solving before acknowledging.** Jumping to the fix without showing you understood makes even a correct answer feel dismissive. Restate the problem first.
- **Over-apologizing or over-promising.** "So so sorry" repeated, or a timeline you can't hold, erodes trust faster than honest limits. Acknowledge the impact, commit only to what you control.
- **Going silent on a delay.** Absence reads as neglect. Proactively update the customer before the deadline slips, even if the news is "still working on it."
- **Saying no with no alternative.** A flat denial ends the relationship; a denial plus the nearest thing you *can* do preserves it. Always pair a no with an offer.
- **Debating in public.** Arguing details on a public review or social post amplifies the conflict. Reply briefly with empathy and move specifics to a private channel.

## Verification Checklist

- [ ] Tone matches the situation (empathetic where the customer is frustrated or the company erred; crisp where it's simple).
- [ ] Message follows the greeting → understanding → action → expectation → close structure.
- [ ] The reply is adapted to its channel (email/chat/phone/social conventions).
- [ ] The customer's problem is restated before the solution is offered.
- [ ] Difficult content (complaint/delay/denial/pricing) follows the deliberate handling for its type, with an alternative where saying no.
